...
const SellerOrderDetailScreen = ({ navigation, route }) => {
  ...
  const getCurrentSellerTransferData = () => {
    ...
    console.log('Getting transfer data for seller:', { currentSellerId, currentStoreName });
    ...
    if (!transferData.isMultiSeller) {
      ...
      if (transferData.transferProofs) {
        transferProof = transferData.transferProofs[currentStoreName] || 
    ...
    const currentSellerInfo = transferData.sellerInfo?.find(info => 
      info.storeName === currentStoreName
    );
    ...
    return {
      ...
      adminFee: order.paymentMethod === 'cod' ? 0 : Math.round(currentSellerAmount * 0.015), // 1.5% admin fee, free for COD
  ...
  const refreshOrderData = async () => {
    try {
      setRefreshing(true);
      console.log('Refreshing order data for order ID:', order.id);
      ...
      if (orderSnap.exists()) {
        ...
        console.log('Updated sellerTransferData:', updatedOrder.sellerTransferData);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryRow}>
              ...
              <Text style={styles.summaryValue}>
                {order.paymentMethod === 'cod' ? formatPrice(0) : formatPrice(order.adminFee || 0)}
              </Text>
            </View>
        ...
        {(order.status === 'processing' || order.sellerTransferData || order.sellerTransferStatus === 'completed') && (() => {
          const transferData = getCurrentSellerTransferData();
          console.log('Transfer section - transferData:', transferData);
          console.log('Transfer section - order.sellerTransferData:', order.sellerTransferData);
          
          // Use either processed data or raw data as fallback
          const finalTransferData = transferData || order.sellerTransferData;
          console.log('Transfer section - finalTransferData:', finalTransferData);
              ...
              <View style={styles.transferProofCard}>
                {finalTransferData ? (
                  <>
                    <View style={styles.transferProofHeader}>
                      ...
                      {finalTransferData.transferredAt && (
                        <Text style={styles.transferDate}>
                          {formatDate(new Date(finalTransferData.transferredAt.seconds * 1000).toISOString())}
                        </Text>
                      )}
                    </View>

                    {/* Seller Bank Info */}
                    {(finalTransferData.sellerInfo || (finalTransferData.sellerInfo && finalTransferData.sellerInfo[0])) && (
                      <View style={styles.sellerBankInfo}>
                        <Text style={styles.sellerBankTitle}>Transfer ke Rekening:</Text>
                        <View style={styles.bankDetails}>
                          <View style={styles.bankRow}>
                            <Text style={styles.bankLabel}>Bank:</Text>
                            <Text style={styles.bankValue}>{(finalTransferData.sellerInfo?.bankName || finalTransferData.sellerInfo?.[0]?.bankName)}</Text>
                          </View>
                          <View style={styles.bankRow}>
                            <Text style={styles.bankLabel}>No. Rekening:</Text>
                            <Text style={styles.bankValue}>{(finalTransferData.sellerInfo?.accountNumber || finalTransferData.sellerInfo?.[0]?.accountNumber)}</Text>
                          </View>
                          <View style={styles.bankRow}>
                            <Text style={styles.bankLabel}>Nama Pemilik:</Text>
                            <Text style={styles.bankValue}>{(finalTransferData.sellerInfo?.accountName || finalTransferData.sellerInfo?.[0]?.accountName)}</Text>
                          </View>
                        </View>
                      </View>
                    )}
                    
                    {(finalTransferData.transferProof || (finalTransferData.transferProofs && Object.keys(finalTransferData.transferProofs).length > 0)) && (
                      <TouchableOpacity 
                        style={styles.transferProofImageContainer}
                        onPress={() => {
                          // Try to get transfer proof from multiple sources
                          const proofUri = finalTransferData.transferProof || 
                                         (finalTransferData.transferProofs && Object.values(finalTransferData.transferProofs)[0]);
                          console.log('Attempting to view transfer proof:', proofUri);
                          handleViewTransferProof(proofUri);
                        }}
                      >
                        <Image 
                          source={{ 
                            uri: finalTransferData.transferProof || 
                                 (finalTransferData.transferProofs && Object.values(finalTransferData.transferProofs)[0])
                          }} 
                    ...
                    <View style={styles.transferAmountInfo}>
                      ...
                      <View style={styles.transferAmountRow}>
                        ...
                        <Text style={styles.transferAmountValue}>
                          {order.paymentMethod === 'cod' ? formatPrice(0) : formatPrice(finalTransferData.adminFee || 0)}
                        </Text>
                      </View>
                      <View style={styles.transferAmountRow}>
                        ...
                        <Text style={[styles.transferAmountValue, styles.receivedAmount]}>
                          {order.paymentMethod === 'cod' 
                            ? formatPrice(finalTransferData.sellerAmount || 0) 
                            : formatPrice((finalTransferData.sellerAmount || 0) - (finalTransferData.adminFee || 0))
                          }
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={styles.transferVerificationSection}>
                      {order.status === 'processing' && !finalTransferData.isVerified && finalTransferData.verificationStatus !== 'verified' ? (
                      ...
                      ) : (finalTransferData.isVerified || finalTransferData.verificationStatus === 'verified') ? (
                        <View style={styles.verificationCompleted}>
                          ...
                          {finalTransferData.verifiedAt && (
                            <Text style={styles.verificationDate}>
                              Diverifikasi pada: {formatDate(new Date(finalTransferData.verifiedAt.seconds * 1000).toISOString())}
                            </Text>
                          )}
                        </View>
                      ) : finalTransferData.verificationStatus === 'rejected' ? (
                        <View style={styles.verificationRejected}>
                          ...
                          {finalTransferData.verifiedAt && (
                            <Text style={styles.verificationDate}>
                              Dilaporkan pada: {formatDate(new Date(finalTransferData.verifiedAt.seconds * 1000).toISOString())}
                            </Text>
                          )}
                        </View>
                      ) : (
                  ...
                  <View style={styles.transferNotFound}>
                    ...
                    <Text style={styles.transferNotFoundText}>Menunggu transfer dari admin...</Text>
                    <Text style={styles.transferNotFoundSubText}>
                      Admin akan melakukan transfer setelah pembayaran terverifikasi.
                    </Text>
                  </View>
                )}
              </View>
      ...
      <View style={styles.bottomActions}>
        ...
        {order.status === 'processing' && (
          <TouchableOpacity 
            ...
            onPress={order.paymentMethod === 'cod' ? handleShipCODOrder : handleShipOrder}
          >
        ...
        {order.status === 'cod_shipped' && (
          <View style={styles.waitingForCustomerCard}>
            ...
            <Text style={styles.waitingForCustomerTitle}>Menunggu Konfirmasi Pembeli</Text>
            <Text style={styles.waitingForCustomerDescription}>
              Pesanan sudah dikirim. Pembeli akan mengkonfirmasi setelah menerima barang dan melakukan pembayaran COD.
            </Text>
          </View>
        )}
      </View>

      {/* Tracking Number Modal - Only for non-COD orders */}
      <Modal
        visible={showTrackingModal && order.paymentMethod !== 'cod'}
        transparent={true}
        animationType="fade"
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <TextInput
              ...
              autoFocus={true}
              returnKeyType="done"
              onSubmitEditing={handleSubmitTracking}
            />
