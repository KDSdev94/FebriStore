...
const SellerOrdersScreen = ({ navigation }) => {
  ...
  const loadOrders = async () => {
    try {
      ...
      Alert.alert('Error', 'Terjadi kesalahan saat memuat pesanan');
    } finally {
      setLoading(false);
    }
  };
        ...
        { text: 'Batal', style: 'cancel' },
        {
          ...
          onPress: async (trackingNumber) => {
            if (trackingNumber && trackingNumber.trim()) {
              try {
                const result = await orderService.addTrackingNumber(orderId, trackingNumber.trim());
                if (result.success) {
                  // Update status to shipped
                  const statusResult = await orderService.updateOrderStatus(orderId, 'shipped');
                  if (statusResult.success) {
                    // Reload orders to get updated data
                    await loadOrders();
                    Alert.alert('Berhasil', 'Pesanan berhasil dikirim dengan nomor resi: ' + trackingNumber.trim());
                  } else {
                    Alert.alert('Error', 'Nomor resi ditambahkan tapi gagal mengupdate status pesanan');
                  }
                } else {
                ...
                Alert.alert('Error', 'Terjadi kesalahan saat menambahkan nomor resi');
              }
            } else {
  ...
  const filteredOrders = selectedTab === 'all' 
    ? orders 
    : selectedTab === 'new_orders'
      ? orders.filter(order => order.status === 'pending_verification' || order.status === 'cod_confirmed')
    : selectedTab === 'processing'
      ? orders.filter(order => order.status === 'processing' || order.status === 'cod_processing')
    : selectedTab === 'shipped'
      ? orders.filter(order => order.status === 'shipped' || order.status === 'cod_shipped')
    : selectedTab === 'delivered'
      ? orders.filter(order => order.status === 'delivered' || order.status === 'cod_delivered')
    ...
    >
      ...
      <View style={styles.customerSection}>
        <View style={styles.customerInfo}>
          <View style={styles.customerAvatarContainer}>
            <Image
              source={{ 
                uri: item.userAvatar || item.userProfileImage || imageService.generatePlaceholderAvatar(
                  item.userName || item.customerName || item.shippingAddress?.name || 'Customer', 
                  'buyer'
                )
              }}
            ...
            <View key={index} style={styles.itemPreviewRow}>
              <View style={styles.itemImageContainer}>
                {(orderItem.image || orderItem.productImage) ? (
                  ...
                  <View style={styles.itemImagePlaceholder}>
                    <MaterialCommunityIcons name="image" size={20} color={COLORS.textLight} />
                  </View>
                )}
              </View>
              <View style={styles.itemPreviewDetails}>
                ...
                <View style={styles.itemPreviewMeta}>
                  <Text style={styles.itemPreviewQuantity}>Qty: {orderItem.quantity}</Text>
        ...
        <View style={styles.shippingDetails}>
          ...
          <Text style={styles.shippingAddress} numberOfLines={2}>
            {typeof item.shippingAddress === 'string' 
              ...
              : `${item.shippingAddress?.name || ''} - ${item.shippingAddress?.address || ''}, ${item.shippingAddress?.city || ''}`
            }
          </Text>
        </View>
      </View>
      ...
      <View style={styles.orderFooter}>
        <View style={styles.totalSection}>
          ...
          <Text style={styles.totalAmount}>{formatPrice(item.totalAmount || item.total)}</Text>
        </View>
        ...
        <View style={styles.actionButtons}>
          {item.status === 'pending_verification' && (
            <TouchableOpacity 
              ...
              onPress={(e) => {
                e.stopPropagation();
                handleUpdateOrderStatus(item.id, 'processing');
              }}
            >
              <MaterialCommunityIcons name="check" size={16} color={COLORS.card} />
          ...
          {item.status === 'processing' && (
            ...
            >
              <MaterialCommunityIcons name="truck-delivery" size={16} color={COLORS.card} />
          ...
          {item.status === 'shipped' && (
            ...
            >
              <MaterialCommunityIcons name="check-circle" size={16} color={COLORS.card} />
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {orders.filter(o => o.status === 'pending_verification' || o.status === 'cod_confirmed').length}
          </Text>
          <Text style={styles.statLabel}>Baru</Text>
        </View>
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {orders.filter(o => o.status === 'processing' || o.status === 'cod_processing').length}
          </Text>
          <Text style={styles.statLabel}>Proses</Text>
        </View>
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {orders.filter(o => o.status === 'shipped' || o.status === 'cod_shipped').length}
          </Text>
          <Text style={styles.statLabel}>Kirim</Text>
        </View>
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {orders.filter(o => o.status === 'delivered' || o.status === 'cod_delivered').length}
          </Text>
