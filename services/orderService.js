...
export const orderService = {
  ...
  async createOrder(orderData) {
    try {
      ...
      console.log('Creating order with cleaned data:', JSON.stringify(newOrder, null, 2));
  ...
  async getOrdersBySeller(sellerId) {
    try {
      // Get all orders first, then filter by seller items
      const q = query(
      ...
      for (const orderDoc of snapshot.docs) {
        ...
        const sellerItems = orderData.items?.filter(item => 
          item.sellerId === sellerId || item.storeId === sellerId
        ) || [];
        ...
        let buyerInfo = {
          name: orderData.userName || orderData.customerName || 'Unknown Customer',
          email: orderData.userEmail || orderData.customerEmail || 'No email',
          phone: orderData.userPhone || orderData.customerPhone || orderData.shippingAddress?.phone || '',
          avatar: null,
          profileImage: null
        };
        
        // Try to get more complete user data from users collection
        if (orderData.userId) {
          try {
            ...
            if (buyerSnap.exists()) {
              ...
              buyerInfo = {
                ...
                profileImage: buyerData.profileImage || buyerData.avatar || null
              };
            }
          } catch (error) {
        ...
        // Enrich seller items with complete product data
            ...
            try {
              ...
              if (productSnap.exists()) {
                ...
                return {
                  ...
                  image: item.image || productData.images?.[0] || productData.imageUrl,
                  images: item.images || productData.images || [],
                  price: item.price || productData.price,
                  category: productData.category,
                  description: productData.description,
                  storeName: item.storeName || productData.storeName || item.sellerName,
        ...
        const sellerSubtotal = enrichedItems.reduce((sum, item) => 
          sum + (item.price * item.quantity), 0
        );
        
        // Debug logging
        console.log('Order buyer info:', {
          orderId: orderData.id,
          originalUserName: orderData.userName,
          originalUserEmail: orderData.userEmail,
        ...
        const formattedOrder = {
          id: orderData.id,
          orderNumber: orderData.orderNumber || `ORD-${orderData.id.substring(0, 8).toUpperCase()}`,
          ...
          adminVerificationStatus: orderData.adminVerificationStatus || 'pending',
          ...
          if (item.selectedVariant && product.variants) {
            ...
            const updatedVariants = product.variants.map(variant => {
              if (variant.name === item.selectedVariant.name) {
                ...
                const newVariantStock = Math.max(0, currentStock - item.quantity);
        ...
        if (orderData.userId) {
          try {
            ...
            if (userSnap.exists()) {
              ...
              orderData.userName = userData.name || userData.fullName || orderData.userName || 'Unknown Customer';
              orderData.userEmail = userData.email || orderData.userEmail || 'No Email';
            ...
            console.log('Error getting user data for order:', orderData.id, error);
            // Keep existing data or set default
            orderData.userName = orderData.userName || 'Unknown Customer';
            orderData.userEmail = orderData.userEmail || 'No Email';
          }
