...
export const OrderProvider = ({ children }) => {
  ...
  const updateOrderStatus = async (orderId, newStatus, additionalData = {}) => {
    try {
      ...
      if (result.success) {
        ...
        return {
          success: false,
          error: result.error || "Gagal memperbarui status pesanan",
        };
      }
    } catch (error) {
  ...
  const updatePaymentProof = async (orderId, paymentProofUri) => {
    try {
      ...
      const result = await orderService.updatePaymentProof(
        orderId,
        paymentProofUri
      );
  ...
  // DANGEROUS: Delete all orders in database by buyer
  const deleteAllOrdersByBuyer = async (userId) => {
    try {
      const result = await orderService.deleteOrdersByBuyer(userId);
      if (result.success) {
        await loadOrders();
      }
      return result;
    } catch (error) {
      console.error("Error deleting orders by buyer:", error);
      return { success: false, error: "Gagal menghapus semua pesanan pembeli" };
    }
  };

  // DANGEROUS: Delete all orders in database that relate to seller
  const deleteAllOrdersBySeller = async (sellerId) => {
    try {
      const result = await orderService.deleteOrdersBySeller(sellerId);
      if (result.success) {
        await loadOrders();
      }
      return result;
    } catch (error) {
      console.error("Error deleting orders by seller:", error);
      return { success: false, error: "Gagal menghapus pesanan seller" };
    }
  };

  // VERY DANGEROUS: Admin deletes all orders in database
  const deleteAllOrdersAdmin = async () => {
    try {
      // Prefer adminService if later we add audit logging, etc.
      const snapshot = await adminService.getAllOrders();
      if (!snapshot.success) return { success: false, error: snapshot.error };
      let successCount = 0;
      for (const o of snapshot.orders) {
        const res = await orderService.updateOrderStatus(o.id, "deleted", {
      ...
      console.error("Error deleting all orders (admin):", error);
      return { success: false, error: "Gagal menghapus semua pesanan (admin)" };
    }
  };
  ...
  const verifyPayment = async (orderId, isApproved, adminNotes = "") => {
    try {
      ...
      if (result.success) {
        ...
        return {
          success: false,
          error: result.error || "Gagal memverifikasi pembayaran",
        };
      }
    } catch (error) {
